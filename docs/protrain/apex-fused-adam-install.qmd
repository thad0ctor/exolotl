...
If the matching toolkit lives in a conda prefix rather than `/usr/local/cuda`,
point the build at that prefix explicitly. Some conda CUDA packages place
library headers under `targets/x86_64-linux/include`, so expose that directory
through `CPATH` as well:

```bash
export CUDA_HOME=/path/to/cuda-13.0-prefix
...
The protrain phase 2 PR validation environment uses a torch `cu130` wheel on a
system whose default `/usr/local/cuda` reports CUDA 13.2. Apex was validated by
installing a CUDA 13.0 compiler/libraries prefix, building NVIDIA Apex from
source with `APEX_CPP_EXT=1 APEX_CUDA_EXT=1`, and running both a direct
`FusedAdam` CUDA step and an Axolotl `optimizer: adamw_apex_fused` smoke.
The default system toolkit still does not match torch; reproducible builds
must set `CUDA_HOME` to the matching toolkit prefix before installing Apex.
